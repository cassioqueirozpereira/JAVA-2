...
public class Equipe extends Thread {
    ...
    // Realiza atomicamente as operações de decremento do latch, incremento do número de pacotes que a equipe impacotou e libera (uma) trava sobre o semáforo "pool_fita".
    ...
    public void prepararEmpacotadores() {
    ...
    // Para cada empregador, se houver pacote disponível para empacotar, decrementa pool_pacotes, dispara uma thread (Empacotador) para realizar o trabalho de empacotamento. Do contrário, não faz nada.
    ...
    private boolean empacotar(int nr_produtos) {
        Thread thread;
        int thread_Criadas = pool_produtos.retirarProdutos(nr_produtos); // O número de threads a ser criado para empacotar é igual ao número de produtos retirados pela equipe.
        boolean controle;
        if(thread_Criadas == 0) { // Não há produtos disponíveis para empacotamento.
            liberarFitasDesnecessarias(nr_produtos); // Devolve todas as fitas pegas
            return false;
        } else {
            for(int i = 1; i <= thread_Criadas; i++) {
                thread = new Thread(empacotadores.get(i - 1));
                thread.setPriority(Thread.currentThread().getPriority() + 2);
                thread.start(); // Inicia a thread (Empacotador).
            }

            liberarFitasDesnecessarias(nr_produtos - thread_Criadas); // Devolve todas as fitas excedentes.
    ...
    public void run() {
        try {
            ...
            do {
                ...
                controle = empacotar(nr_integrantes); // Cada integrande da equipe empacota um produto.
            } while(controle);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
