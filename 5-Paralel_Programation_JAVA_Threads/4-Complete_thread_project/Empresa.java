...
public class Empresa {
    ...
    private final ArrayList <Equipe> turno; // Conjunto de equipe de empacotadores. Cada equipe possui 2 ou mais empacotadores e corresponde a uma thread.
    ...
    public Empresa(int nr_fitas, int pool_empacotadores, int nr_max_equipes, int max_prod_empacotar) throws InterruptedException {
        if ((nr_fitas < 1) || (pool_empacotadores < 2) || (nr_max_equipes < 1) || (max_prod_empacotar < 1)) throw new IllegalArgumentException("Argumentos ilegais utilizados no construtor da Empresa.");
        else {
    ...
    private void criarEquipes(int nr_fitas) {
        ...
        do { // Cria as equipes com um número aleatório de integrantes e as adiciona em "turno".
            ...
            eqp = new Equipe("Equipe [" + String.valueOf(i) + "]", nr_emp_eqp, pool_fita, pool_produtos);
            turno.add(eqp);
            empacotadores_disponiveis = empacotadores_disponiveis - nr_emp_eqp;
            i++;
        } while((i < nr_max_equipes) && (empacotadores_disponiveis >= 2)); // A última equipe recebe todos os empregados restantes. Se o número de empregados for maior que o número de fitas, são alocados nr_fitas empregados (caso contrário, a equipe iria solicitar um número de permissões superior ou número de recursos do semáforo e ficaria bloqueada).

        if(empacotadores_disponiveis > 0) {
            if(nr_fitas >= empacotadores_disponiveis)
                eqp = new Equipe("Equipe [" + String.valueOf(i) + "]", empacotadores_disponiveis, pool_fita, pool_produtos);
            else
                eqp = new Equipe("Equipe [" + String.valueOf(i) + "]", nr_fitas, pool_fita, pool_produtos);
            turno.add(eqp);
        }
    }
    ...
    private final int comporEquipe(int empacotadores_disponiveis) {
        ...
        if(empacotadores_disponiveis > 2)
            return random.nextInt(empacotadores_disponiveis / nr_max_equipes) + 2;
        else
